---
title: Epimethean Challenge
toc: false
---

Welcome to the documentation for **Epimethean Challenge**, a computer-moderated,
hidden-movement, play-by-email science fiction strategy game.

The goal of the player is the control of a star cluster consisting of 100 solar
systems. All of the systems have from one to ten planets each, but a maximum of
10 systems are inhabited; each of those systems having one inhabited world.

Each of the inhabited planets has a maximum of 25 nations; the players on a
planet rule the nations individually, and together constitute a "race."

Each game will have an entirely different set of locations for the 100 solar
systems, so that no two games will be entirely alike.

This documentation follows the [Diátaxis](https://diataxis.fr) framework. Pick
the section that matches what you need:

{{< cards >}}
  {{< card link="tutorial/" title="Tutorials" subtitle="Learn by doing — start here if you're new." >}}
  {{< card link="how-to/" title="How-to guides" subtitle="Step-by-step recipes for specific tasks." >}}
  {{< card link="reference/" title="Reference" subtitle="Precise, authoritative descriptions of the rules." >}}
  {{< card link="explanation/" title="Explanation" subtitle="Background and the reasoning behind the rules." >}}
{{< /cards >}}

## Working Index

- 1 - [INTRODUCTION](#)
- 2 - ~~HISTORY OF THE EMPYREAN CLUSTER~~
- 3 - [GAME SET UP]({{< relref "reference/game-setup.md" >}})
- 3.1 - [Star Lists]({{< relref "reference/game-setup.md#star-lists" >}})
- 3.2 - [Solar Systems]({{< relref "reference/game-setup.md#solar-systems" >}})
- 3.2.1 - [Orbits]({{< relref "reference/game-setup.md#orbits" >}})
- 3.2.2 - [Home System List]({{< relref "reference/game-setup.md#home-system" >}})
- 3.3 - [Planets]({{< relref "reference/planets.md" >}})
- 3.3.1 - [Types]({{< relref "reference/planets.md#types" >}})
- 3.3.1.1 - [Gas Giants]({{< relref "reference/planets.md#types" >}})
- 3.3.1.2 - [Terrestrials]({{< relref "reference/planets.md#types" >}})
- 3.3.1.3 - [Asteroids]({{< relref "reference/planets.md#types" >}})
- 3.3.2 - [Habitability]({{< relref "reference/habitability.md" >}})
- 3.3.3 - [Natural Resources]({{< relref "reference/planets.md#natural-resources" >}})
- 3.4 - [Time Scale]({{< relref "reference/game-setup.md#time-scale" >}})
- 3.5 - ~~Initial Print-out~~
- 3.6 - [Initial Turn Sheet]({{< relref "reference/game-setup.md#initial-turn-report" >}})
- 4 - [BASIC UNITS]({{< relref "reference/units.md" >}})
- 4.1 - [Description of basic units]({{< relref "reference/units.md#basic-units" >}})
- 4.2 - [Population Units]({{< relref "reference/units.md#population-units" >}})
- 4.2.1 - [Orders applicable to population units]({{< relref "reference/population.md#orders" >}})
- 4.2.2 - [Population units on ships]({{< relref "reference/population.md#ships" >}})
- 4.2.3 - [Population Charts]({{< relref "reference/population.md#population-classes" >}})
- 4.3 - [Weapons]({{< relref "reference/units.md#weapons" >}})
- 4.4 - [Production Units]({{< relref "reference/units.md#production" >}})
- 4.4.1 - [Production Chart]({{< relref "reference/units.md#production" >}})
- 4.4.2 - [Farms]({{< relref "reference/farming.md" >}})
- 4.4.3 - [Mines]({{< relref "reference/mining.md" >}})
- 4.4.4 - [Factories]({{< relref "reference/units.md#production" >}})
- 4.5 - [Miscellaneous units]({{< relref "reference/units.md#miscellaneous" >}})
- 4.5.1 - [Automation]({{< relref "reference/units.md#miscellaneous" >}})
- 4.5.2 - [Consumer Goods and Food]({{< relref "reference/units.md#materials" >}})
- 4.5.2.1 - [Starvation Rule]({{< relref "reference/food.md#starvation" >}})
- 4.5.3 - [Life Support Units]({{< relref "reference/units.md#miscellaneous" >}})
- 4.5.4 - [Hyper Engines]({{< relref "reference/units.md#miscellaneous" >}})
- 4.5.5 - [Space Drives]({{< relref "reference/units.md#miscellaneous" >}})
- 4.5.6 - [Sensors]({{< relref "reference/units.md#miscellaneous" >}})
- 4.5.6.1 - Probes
- 4.5.7 - [Transports]({{< relref "reference/units.md#miscellaneous" >}})
- 4.5.8 - [Structural Units]({{< relref "reference/units.md#miscellaneous" >}})
- 4.5.9 - [Light Structural Units]({{< relref "reference/units.md#miscellaneous" >}})
- 5 - COLONIES AND SHIPS
- 5.1 - Chart
- 5.2 - Establishment
- 6 - MANUFACTURING
- 6.1 - Factory Groups
- 6.2 - Labor
- 6.3 - [Shortages]({{< relref "reference/shortages.md" >}})
- 6.4 - "Cost" Chart
- 6.5 - No. of Factory Units Required
- 6.6 - Retooling
- 6.7 - Assembling
- 6.8 - Dis-assembling
- 7 - TECHNOLOGICAL ADVANCEMENT
- 8 - TRADE
- 8.1 - Trade Stations
- 8.2 - Home Planet Markets
- 9 - EXPLORATION
- 9.1 - Ship Movement
- 9.1.1 - Interplanetary Movement
- 9.1.2 - Interstellar Movement
- 9.2 - Probes
- 9.3 - Surveys
- 10 - [REBELLION]({{< relref "reference/rebellion.md" >}})
- 10.1 - [Rebellion]({{< relref "reference/rebellion.md#when-rebellion-occurs" >}})
- 10.2 - [Independent Colonies and Ships]({{< relref "reference/rebellion.md#independent-ships-and-colonies" >}})
- 11 - COMBAT
- 11.1 - Description
- 11.2 - Military Supplies
- 11.3 - Percentage of Commitment
- 11.4 - Attack
- 11.4.1 - Attack Orders
- 11.4.2 - Raids and Invasions
- 11.4.2.1 - Assault craft
- 11.4.2.2 - Assault weapons
- 11.4.2.3 - Transports
- 11.4.2.4 - Military robots
- 11.4.3 - Bombardment
- 11.4.3.1 - Bombardment by colonies
- 11.4.3.2 - Bombardment by ships
- 11.5 - Defense
- 11.5.1 - Defense against soldier units
- 11.5.2 - Defense against bombardment
- 11.5.3 - Defense while invading
- 11.5.4 - Defense against raids
- 11.5.5 - Defense support
- 11.6 - Combat Factor
- 11.7 - Casualties
- 11.7.1 - Invasion casualties
- 11.7.2 - Troop transport casualties
- 11.7.3 - Bombardment casualties
- 11.7.4 - Raiding casualties
- 11.8 - Damage
- 11.8.1 - Energy Weapons
- 11.8.1.1 - Description
- 11.8.1.2 - Energy weapons against ships/colonies
- 11.8.1.3 - Energy weapons against transports and assault craft
- 11.8.1.4 - Energy weapons against missiles
- 11.8.1.5 - Energy shields
- 11.8.2 - Missiles
- 11.8.2.1 - Missile launchers
- 11.8.3 - Anti-missiles
- 11.8.3.1 - Description
- 11.8.3.2 - Against missiles
- 11.8.3.3 - Against transports and assault craft
- 11.9 - Ship Movement During Combat
- 11.10 - End of Round
- 11.11 - Surrender
- 11.12 - Captured Colonies
- 11.12.1 - Captured population units
- 11.13 - Combat and Ship/Colony TL
- 12 - ESPIONAGE
- 12.1 - Rebel Quantity and Type
- 12.2 - Convert Rebels
- 12.3 - Uncover Foreign Spy
- 12.4 - Suppress Foreign Spy
- 12.5 - Incite Rebellion
- 12.6 - Obtain Secrets
- 13 - CONTROL OF PLANETS
- 13.1 - Taking Control
- 13.2 - Controlled Planets
- 13.3 - Relinquishing Planets
- 14 - COMMUNICATION
- 14.1 - News Service
- 14.2 - Diplomatic Messages
- 14.3 - Enforcement of Treaties and Agreements
- 15 - [VICTORY CONDITIONS]({{< relref "reference/victory-conditions.md" >}})
- 16 - SEQUENCE OF TURN EXECUTION
- 17 - WRITING ORDERS
- 17.1 - General Rules
- 17.2 - List of Orders
- 17.2.1 - Combat orders
- 17.2.2 - Set Up Orders
- 17.2.3 - Assembly orders
- 17.2.4 - Dis-assembly orders
- 17.2.5 - Build Change Orders
- 17.2.6 - Transfer Orders
- 17.2.7 - Mining Change Orders
- 17.2.8 - Market Order
- 17.2.9 - Survey Orders
- 17.2.10 - Probe Orders
- 17.2.11 - Spy Orders
- 17.2.12 - News Release
- 17.2.13 - Jump Orders
- 17.2.14 - Draft Orders
- 17.2.15 - Pay Orders
- 17.2.16 - Ration Orders
- 17.2.17 - Control Orders
- 17.2.18 - Un-Control Orders
- 17.2.19 - Naming Orders
- 17.2.19.1 - Naming Planets
- 17.2.19.2 - Naming Ships and Colonies
- 17.2.20 - Trade Station Orders
- 17.2.21 - Colonizing Permission
- Appendix A
- A.1 - APPROXIMATION
- A.2 - FRACTIONS
- A.3 - GROUPS
- A.4 - HABITABILITY NUMBER
- A.5 - HYPER SPACE
- A.6 - MASS
- A.7 - STORAGE
- Appendix B
- B.1 - QUANTITIES REPRESENTED BY UNITS
- Appendix C: Sample Documents
- C.1 - Orders File
- C.2 - Turn Report
- Colophon

## Additional Documents
- [GLOSSARY]({{< relref "reference/glossary.md" >}})
